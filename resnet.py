...
model_type = 'RestNet%dv%d' % (depth, version)

# load the CIFAR10 data
(x_train, y_train), (x_test, y_test) = cifar10.load_data()

# inptu image dimensions.
...
def resnet_layer(inputs,
    ...
    :argument
        :param inputs: (tensor) input tensor from
...
def resnet_v1(input_shape, depth, num_classes=10):
    ...
    same number of filtres.
    Feature maps sizes:
    stage 0: 32x32, 16
    stage 1: 16x16, 32
    stage 2: 8x8, 64
    The Number of parameters is apporx the same as Table 6 of [a]:
    ResNet20 0.27M
    ResNet32 0.46M
    ResNet44 0.66M
    ResNet56 0.85M
    ...
    # instantiate the stack of residual blocks
    ...
    # v1 does not use NB after last shortcut connection-ReLU
...
def resnet_v2(input_shape, depth, num_classes=10):
    ...
    First shortcut connection per layer is 1x 1 conv2D.
    Second and onwards shortcut connection is indentity.
    ...
    :argument
        :param input_shape: (tensor) shape of input image tensor
        :param depth: (int) number of core convolutinal layers
        :param num_classes: (int) number of classes (CIFAR10 has 10)
    :return:
        model: (Model) Keras model instance
    """
...
model_name = 'cifar10_{}_model.epoch{:03d}.h5'.format(model_type, epochs)
